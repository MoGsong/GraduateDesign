...
import numpy as np
import os.path
from load_datapro import BCI_IV_2a_mat
...
def Kfoldtrain(class_id=1,subject_id=3,nb_classes=2,modelnum=8,opt=1,kfoldnum=5,epochs=500,Es=True,patience=100,extra_train=False,extra_epoch=50,resamplie=True):
    ...
    #导入仪训练数据
    # (train_x,train_y),(val_x,val_y),(test_x,test_y) = BCI_IV_2a_mat(class_id,subject_id,classnum=classnum,resamplie=True)
    ...
    # enc = OneHotEncoder()    # enc.fit(train_y)
    ...
    while fold<=kfoldnum:
        ...
        train, val = list(kfold.split(train_x, train_y))[fold - 1]
        ...
        if nb_classes == 2:
            checkpointer_file = os.path.join(
                    os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d/' % (nb_classes) + makers[class_id], check_file)
            checkpointer_filefinal = os.path.join(
                    os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d/' % (nb_classes) + makers[class_id], check_filefinal)
            file_name = os.path.join(
                    os.getcwd() + "/info/" + modelname + '/classnum%d/' % (nb_classes) + makers[class_id], "info_A{:02d}.mat".format(subject_id))
            if not os.path.exists(checkpointer_file):
                os.makedirs(os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d/' % (nb_classes) + makers[class_id],
                            exist_ok=True)
                os.makedirs(os.getcwd() + "/info/" + modelname + '/classnum%d/' % (nb_classes) + makers[class_id],
                            exist_ok=True)
            else:
            ...
            checkpointer_file = os.path.join(
                    os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d' % (nb_classes),check_file)
            checkpointer_filefinal = os.path.join(
                    os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d' % (nb_classes),check_filefinal)
            file_name = os.path.join(
                    os.getcwd() + "/info/" + modelname + '/classnum%d/' % (nb_classes) , "info_A{:02d}.mat".format(subject_id))
            if not os.path.exists(checkpointer_file):
                os.makedirs(os.getcwd() + "/kcheckpoint/" + modelname + '/classnum%d/' % (nb_classes),
                            exist_ok=True)
                os.makedirs(os.getcwd() + "/info/" + modelname + '/classnum%d/' % (nb_classes),
                            exist_ok=True)
            else:
                model.load_weights(checkpointer_file)
        ...
        fittedModel = model.fit(X, Y, batch_size = 64, epochs = epochs,
                                verbose = 2, validation_data=(test_x, test_y),  # # X_val, Y_val
                                callbacks=callbacks)
    ...
    if extra_train:
        ...
        if opt==1:
            ...
            optimizer = tf.keras.optimizers.SGD(learning_rate=0.001)
        model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])  #metrics=['categorical_accuracy']

        checkpointer = ModelCheckpoint(checkpointer_filefinal,
                                       monitor='val_accuracy', mode='max', verbose=0,save_best_only=True)
        fittedModel = model.fit(train_x, train_y, batch_size=64 ,epochs=extra_epoch,
        ...
        val_acc_after = fittedModel.history['val_accuracy']
        train_acc_after = fittedModel.history['accuracy']
